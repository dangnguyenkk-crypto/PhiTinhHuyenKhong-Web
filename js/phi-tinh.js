...
// Ghép ý nghĩa riêng của sao với trạng thái vượng-suy hiện tại: cat dùng nghĩa tốt (chia 3 mức: Vượng
// khí/Sinh khí/Sinh khí Viễn theo đúng d=0/1/2 của trangThaiThoiVan), hung dùng nghĩa xấu, còn binh
// (Suy khí/Thoái khí) thì nghĩa tốt đã nhạt dần, chưa tới mức xấu hẳn — cần kích hoạt lại.
// Tham số "tt" là NGUYÊN object trả về từ trangThaiThoiVan() (không phải chỉ tt.loai), để lấy được
// tt.ten phân biệt 3 mức Cát; giữ khả năng nhận chuỗi "cat"/"hung"/"binh" thô (gọi cũ) làm dự phòng.
function yNghiaSaoTheoVan(sao, tt) {
    let sy = SAO_Y_NGHIA[sao];
    let ten = sy.tenKhac ? `${sy.ten} (${sy.tenKhac})` : sy.ten; // tương thích cả 2 dạng dữ liệu (gộp sẵn hoặc tách riêng)
    let loaiTt = (tt && typeof tt === "object") ? tt.loai : tt; // tương thích cả gọi mới (object) và gọi cũ (chuỗi)
    let tenTt = (tt && typeof tt === "object") ? tt.ten : null;
    if (loaiTt === "cat") {
        if (tenTt === "Vượng khí") return `${ten} vượng khí đang dồi dào nhất: ${sy.cat}.`;
        if (tenTt === "Sinh khí") return `${ten} vượng khí đang tích lũy cho vận sau: ${sy.cat}.`;
        if (tenTt === "Sinh khí (Viễn)") return `${ten} đang bắt đầu nhen nhóm tích lũy vượng khí: ${sy.cat}.`;
        return `${ten} đang phát đúng bản chất tốt: ${sy.cat}.`; // dự phòng khi không có tenTt (gọi kiểu cũ truyền chuỗi)
    }
...
function luanGiaiCung(tenCungVi, vanNha, sVan, sSon, sHuong, sNien, sNguyet, sNhat, thangXem, vanHienTai, namXem) {
    ...
    function luanTrongTam(nhan, ky, sao, q, tt, chuDe, loaiBan) {
        ...
        let cau = `<b>${tt.moTa}</b>. ${moTaQuanHeCoNhan(nhanNgan, hanhSao, hanhCung, q.loai)} → <b>${q.nhan}</b> cho ${chuDe}. ${yNghiaSaoTheoVan(sao, tt)}`;
        if (q.loai === "xau") {
    ...
    function rowKhachTinh(nhan, ky, vaiTro, saoKhach) {
        ...
        let giai = `<b>${ttC.moTa}</b>. ${moTaQuanHeCoNhan(nhanDay, HANH_CUA_SAO[saoKhach], hanhCung, qC.loai)} → <b>${qC.nhan}</b>. ${yNghiaSaoTheoVan(saoKhach, ttC)}`;
